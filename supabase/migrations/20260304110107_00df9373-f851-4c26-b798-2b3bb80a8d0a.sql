...
CREATE TABLE public.schools (
    ...
    name TEXT NOT NULL, address TEXT DEFAULT '', state TEXT DEFAULT '', city TEXT DEFAULT '',
    phone TEXT DEFAULT '', logo TEXT, sections TEXT[] DEFAULT ARRAY['A'],
...
CREATE TABLE public.teachers (
    ...
    first_name TEXT NOT NULL, last_name TEXT DEFAULT '', classes TEXT[] DEFAULT '{}',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE public.students (
    ...
    name TEXT NOT NULL, father_name TEXT DEFAULT '', class TEXT DEFAULT '', section TEXT DEFAULT 'A',
    roll_no TEXT DEFAULT '', xp INTEGER DEFAULT 0, progress INTEGER DEFAULT 0,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE public.user_security (
    ...
    pin TEXT NOT NULL, security_question TEXT NOT NULL, security_answer TEXT NOT NULL,
...
CREATE POLICY "Schools can manage own teachers" ON public.teachers FOR ALL USING (EXISTS (SELECT 1 FROM public.schools WHERE schools.id = teachers.school_id AND schools.user_id = auth.uid()));
...
CREATE POLICY "Schools can manage own students" ON public.students FOR ALL USING (EXISTS (SELECT 1 FROM public.schools WHERE schools.id = students.school_id AND schools.user_id = auth.uid()));
