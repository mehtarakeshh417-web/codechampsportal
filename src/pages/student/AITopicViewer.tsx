...
import {
  ...
  Lightbulb, RefreshCw, ArrowRight, ArrowLeft,
} from "lucide-react";
...
interface AIContent {
  ...
  learn: { heading: string; body: string; highlight?: string; example?: string }[];
  images: { title: string; description: string; emoji: string; caption: string }[];
  activities: { title: string; instructions: string[]; expectedOutput: string; hint: string }[];
  practice: { type: "mcq" | "fill" | "truefalse"; question: string; options?: string[]; answer: string; explanation: string }[];
  quiz: { question: string; options: string[]; answerIndex: number; explanation: string }[];
}

const cacheKey = (id: string) => `topic-ai:${id}`;
const TABS = [
...
const AITopicViewer = () => {
  ...
  const loadContent = useCallback(async (force = false) => {
    ...
    setError(null);
    if (!force) {
      const cached = localStorage.getItem(cacheKey(topicId));
      if (cached) {
        try { setContent(JSON.parse(cached)); return; } catch {}
      }
    }
    setLoading(true);
    try {
      const { data, error: fnErr } = await supabase.functions.invoke("generate-topic-content", {
        body: { topicId, classNumber, title: topicMeta.topic.title },
      });
      if (fnErr) throw fnErr;
      if ((data as any)?.error) throw new Error((data as any).error);
      const c = (data as any).content as AIContent;
      setContent(c);
      try { localStorage.setItem(cacheKey(topicId), JSON.stringify(c)); } catch {}
    } catch (e: any) {
      console.error(e);
      setError(e?.message || "Failed to generate content");
      toast.error("Couldn't load AI content. Try again.");
    } finally {
      setLoading(false);
    }
  }, [topicId, topicMeta, classNumber]);
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => loadContent(true)}
            disabled={loading} className="text-white/40 hover:text-white gap-1">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        {loading && !content && (
          <div className="glass-card p-12 text-center">
            ...
            <p className="text-white/60 font-body">Generating your interactive lesson…</p>
        ...
        {error && !content && (
          <div className="glass-card p-8 text-center">
            ...
            <Button onClick={() => loadContent(true)}>Retry</Button>
          </div>
        )}
        ...
        {content && (
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <TabsContent value="overview"><OverviewTab c={content} /></TabsContent>
              <TabsContent value="learn"><LearnTab c={content} /></TabsContent>
              <TabsContent value="images"><ImagesTab c={content} /></TabsContent>
              <TabsContent value="activities"><ActivitiesTab c={content} /></TabsContent>
              <TabsContent value="practice"><PracticeTab c={content} /></TabsContent>
              <TabsContent value="quiz"><QuizTab c={content} onComplete={toggleComplete} isCompleted={isCompleted} /></TabsContent>
              {lab && (
                <TabsContent value="lab">
                  <LabTab lab={lab} topicTitle={topicMeta.topic.title}
...
const OverviewTab = ({ c }: { c: AIContent }) => (
  <div className="space-y-5">
    <Card>
      <p className="text-white/80 font-body leading-relaxed">{c.overview.introduction}</p>
    </Card>
    <div className="grid md:grid-cols-3 gap-4">
      <Card className="!p-4">
        <div className="flex items-center gap-2 mb-1 text-xs text-white/50 uppercase tracking-wider"><Target className="w-3.5 h-3.5" /> Difficulty</div>
        <div className="font-display text-lg text-white">{c.overview.difficulty}</div>
      </Card>
      <Card className="!p-4">
        <div className="flex items-center gap-2 mb-1 text-xs text-white/50 uppercase tracking-wider"><Clock className="w-3.5 h-3.5" /> Time</div>
        <div className="font-display text-lg text-white">{c.overview.estimatedMinutes} min</div>
      </Card>
      <Card className="!p-4">
        <div className="flex items-center gap-2 mb-1 text-xs text-white/50 uppercase tracking-wider"><Sparkles className="w-3.5 h-3.5" /> Concepts</div>
        <div className="font-display text-lg text-white">{c.overview.keyConcepts.length}</div>
      </Card>
    </div>
    <Card>
      ...
      <ul className="space-y-2">
        {c.overview.objectives.map((o, i) => (
          <li key={i} className="flex gap-3 text-white/70 font-body">
    ...
    <Card>
      ...
      <div className="flex flex-wrap gap-2">
        {c.overview.keyConcepts.map((k, i) => (
...
const ActivitiesTab = ({ c }: { c: AIContent }) => (
...
const PracticeTab = ({ c }: { c: AIContent }) => {
  const [answers, setAnswers] = useState<Record<number, string>>({});
  const [revealed, setRevealed] = useState<Record<number, boolean>>({});

  const check = (i: number, q: any) => {
      ...
      {c.practice.map((q, i) => (
        <Card key={i}>
            ...
            <div className="flex gap-2 mb-3">
              {["True", "False"].map((v) => (
                <button key={v} onClick={() => setAnswers((a) => ({ ...a, [i]: v }))}
                  className={`px-4 py-2 rounded-lg border text-sm ${answers[i] === v
                    ? "bg-primary/20 border-primary/40 text-primary" : "bg-white/5 border-white/10 text-white/70"}`}>
                  {v}
                </button>
              ))}
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {revealed[i] && (
              <p className="text-xs text-white/50 font-body">{q.explanation}</p>
            )}
          </div>
        </Card>
      ))}
...
const QuizTab = ({ c, onComplete, isCompleted }: { c: AIContent; onComplete: () => void; isCompleted: boolean }) => {
  ...
  const score = Object.entries(answers).reduce((acc, [i, ans]) => {
    return acc + (c.quiz[+i]?.answerIndex === ans ? 1 : 0);
  }, 0);

  const submit = () => {
    setSubmitted(true);
    if (score >= Math.ceil(c.quiz.length * 0.6) && !isCompleted) onComplete();
  };
    ...
    return (
      <Card>
        <div className="text-center mb-6">
          ...
          <p className="text-white/60 font-body">{score} / {c.quiz.length} correct</p>
  ...
  return (
    <div className="space-y-4">
      <Card className="!p-4 flex items-center justify-between">
        <div className="text-white/60 text-sm">Question {Object.keys(answers).length}/{c.quiz.length}</div>
      ...
      <Button onClick={submit} disabled={Object.keys(answers).length < c.quiz.length} size="lg" className="w-full">
        Submit Quiz
      </Button>
    </div>
  );
...
const LabTab = ({ lab, topicTitle, onOpen }: { lab: { editor: string; label: string }; topicTitle: string; onOpen: () => void }) => (
